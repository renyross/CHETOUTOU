...
# FIXED RECOMMENDATION BLOCK FOR ALIGNMENT AND SCROLLING
recommendation_block = """        <section class="recommendations-section" style="padding-top: 6rem; border-top: 1px solid #eee; margin-top: 4rem;">
            <div class="container">
                <div class="recommendations-carousel-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2.5rem;">
                    <h2 class="section-title-left" style="margin: 0; font-size: 2rem; font-weight: 700; letter-spacing: -0.5px; color: #111;">Vous aimerez aussi</h2>
...
def update_file(filepath):
    try:
        ...
        # Update Visual Carousel Header for alignment if it exists
        if '<div class="category-visual-header">' in content:
            new_header = """                    <div class="category-visual-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                        <h2 style="margin: 0; font-size: 1.2rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: #111;">Parcourir par type</h2>
                        <div class="category-carousel-nav" style="display: flex; gap: 0.8rem;">
                            <button class="nav-arrow nav-prev" id="cat-prev" aria-label="Précédent" style="width: 36px; height: 36px; border: 1px solid #eee; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: #fff; cursor: pointer; transition: all 0.3s; color: #111;">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
                            </button>
                            <button class="nav-arrow nav-next" id="cat-next" aria-label="Suivant" style="width: 36px; height: 36px; border: 1px solid #eee; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: #fff; cursor: pointer; transition: all 0.3s; color: #111;">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                            </button>
                        </div>
                    </div>"""
            content = re.sub(r'<div class="category-visual-header">.*?</div>\s+</div>', new_header + "\n                </div>", content, flags=re.DOTALL)
            # Fallback if the regex above is too strict
            if '<div class="category-visual-header" style="display: flex;' not in content:
                 content = re.sub(r'<div class="category-visual-header">.*?</div>', new_header, content, flags=re.DOTALL)
...
print(f"Successfully updated alignment and scrolling for {count} files.")
