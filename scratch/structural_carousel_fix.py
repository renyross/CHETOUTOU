...
# IMPROVED RECOMMENDATION BLOCK (Centered + Fixed structure)
recommendation_block = """        <section class="recommendations-section" style="padding-top: 6rem; border-top: 1px solid #eee; margin-top: 4rem;">
            <div class="container">
                <div class="recommendations-carousel-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2.5rem;">
                    <h2 class="section-title-left" style="margin: 0; font-size: 2rem; font-weight: 700; letter-spacing: -0.5px; color: #111; line-height: 1.2;">Vous aimerez aussi</h2>
                    <div class="category-carousel-nav" style="display: flex; gap: 0.8rem; align-items: center;">
                        <button class="nav-arrow nav-prev" id="rec-prev" aria-label="Précédent" style="width: 42px; height: 42px; border: 1px solid #ddd; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: #fff; cursor: pointer; transition: all 0.3s; color: #111;">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
                        </button>
                        <button class="nav-arrow nav-next" id="rec-next" aria-label="Suivant" style="width: 42px; height: 42px; border: 1px solid #ddd; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: #fff; cursor: pointer; transition: all 0.3s; color: #111;">
...
def update_file(filepath):
    try:
        ...
        # 1. Update Recommendations Section
        ...
        # 2. Fix Visual Carousel Structure and Alignment
        # We need to make sure the list is INSIDE the container and header is aligned
        if '<div class="category-visual-container">' in content:
            # First, extract the current list content to preserve it
            list_match = re.search(r'<div class="category-visual-list".*?</div>\s*</div>', content, flags=re.DOTALL)
            if not list_match: # Fallback if already partially broken
                list_match = re.search(r'<div class="category-visual-list".*?</div>', content, flags=re.DOTALL)
            
            if list_match:
                list_content = list_match.group(0)
                # Remove the closing div of the container if it's there
                list_content = list_content.replace('</div>\s*</div>', '</div>')
                ...
                new_header = """                    <div class="category-visual-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
                        ...
                        <div class="category-carousel-nav" style="display: flex; gap: 0.8rem; align-items: center;">
                            <button class="nav-arrow nav-prev" id="cat-prev" aria-label="Précédent" style="width: 38px; height: 38px; border: 1px solid #ddd; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: #fff; cursor: pointer; transition: all 0.3s; color: #111;">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
                            </button>
                            <button class="nav-arrow nav-next" id="cat-next" aria-label="Suivant" style="width: 38px; height: 38px; border: 1px solid #ddd; border-radius: 50%; display: flex; align-items: center; justify-content: center; background: #fff; cursor: pointer; transition: all 0.3s; color: #111;">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                            </button>
                        </div>
                    </div>"""
                
                # Replace the whole container content
...
print(f"Successfully fixed structure and alignment in {count} files.")
